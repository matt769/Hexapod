...
Receiver::Receiver(Hexapod* hexapod) { setRobot(hexapod); }
...
void Receiver::processCommand(const uint8_t cmd) {
  ...
  if (hexapod_->getState() == Hexapod::State::FULL_MANUAL) {
    ...
    Serial.print('\n');
#else
    std::cout << "Manual mode: " << static_cast<int>(mct) << '\t' << "Leg: " << current_leg_idx << '\t'
  ...
  switch (cmd) {
    ...
    case 107:
      ...
      break;
    case 106:
      body_change.R_.setRPYExtr(hexapod_->body_rotation_increment_, 0.0f, 0.0f);
      hexapod_->changeBody(body_change);
      break;
    case 108:
      body_change.R_.setRPYExtr(-hexapod_->body_rotation_increment_, 0.0f, 0.0f);
      ...
      break;
    case 117:
      body_change.R_.setRPYExtr(0.0f, 0.0f, hexapod_->body_rotation_increment_);
      hexapod_->changeBody(body_change);
      break;
    case 111:
      body_change.R_.setRPYExtr(0.0f, 0.0f, -hexapod_->body_rotation_increment_);
