...
/**
 * @details All input should relate to the physical joint used - it will be modified to fit the
 * internal hexapod reference frames based on the offset and flip_axis parameters as required. This
 * constructor may be removed in the near future in favour of more explicit instantiation.
 * @param physical_lower_limit The joint limit in the clockwise direction of the physical joint.
 * Always less than upper.
 * @param physical_upper_limit  The joint limit in the anti-clockwise direction of the physical
 * joint. Always more than lower.
 ...
 */
Joint::Joint(const float physical_lower_limit, const float physical_upper_limit, const float physical_angle,
             const float offset, const bool flip_axis) {
...
Joint Joint::createFromPhysicalAngles(const float physical_lower_limit, const float physical_upper_limit,
                                      const float physical_angle, const float offset, const bool flip_axis) {
  Joint j;
  j.offset_ = offset;
...
Joint Joint::createFromModelAngles(const float model_lower_limit, const float model_upper_limit,
                                   const float model_angle, const float offset, const bool flip_axis) {
  Joint j;
  j.offset_ = offset;
  j.flip_axis_ = !flip_axis ? 1.0 : -1.0;
...
bool Joint::isWithinLimits(const float angle) const {
  return (angle >= lower_limit_ - hexapod::util::eps) && (angle < upper_limit_ + hexapod::util::eps);
}
float Joint::clampToLimits(const float angle) const { return fmax(fmin(angle, upper_limit_), lower_limit_); }
...
float Joint::angle() const { return angle_; }

float Joint::fromPhysicalAngle(const float physical_angle) const { return (physical_angle - offset_) * flip_axis_; }

float Joint::toPhysicalAngle(float model_angle) const { return (flip_axis_ * model_angle) + offset_; }

float Joint::physicalAngle() const { return (flip_axis_ * angle_) + offset_; }

float Joint::toPhysicalAngle() const { return (flip_axis_ * angle_) + offset_; }

void Joint::setFromPhysicalAngle(const float physical_angle) { angle_ = fromPhysicalAngle(physical_angle); }
