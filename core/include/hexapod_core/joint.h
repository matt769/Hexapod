...
/** @class Joint
    ...
    @details This join is intended to represent a joint in the hexapod model. But if there is a physical robot,
     or some other model that needs to map to the hexapod model, then it may be convenient to provide an offset value
     and axis 'flip' indicator to describe the relationship between then so that angles can be converted between the
     alternate joint spaces. In relation to the Joint object, the alternate space will be referred to as 'physical'
      (though it does not have to be).
    No units enforced but other parts of the code expect radians.
    It is a common need to test against angles that aren't the current joint value, which is why most functions have a
     version which operates on the object itself, and a version which operates on a provided value and returns a result
...
class Joint {
 ...
 public:
  ...
  /// @brief 'full' constructor - takes angles from the physical frame - will like change in the near future
...
/** @class JointBuilder
    @brief Utility class to make creating the joints more explicit (I found it easy to get confused between the 2 joint spaces)
 */
class JointBuilder {
private:
    Joint joint;
