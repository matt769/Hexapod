...
#include <ax12.h>             // https://github.com/matt769/DynamixelAX12
#include <build_hexapod.h>    // https://github.com/matt769/Hexapod
#include <hexapod.h>          // https://github.com/matt769/Hexapod
#include <ps4.h>              // https://github.com/matt769/PS4
#include <transformations.h>  // https://github.com/matt769/Hexapod
#include "ps4_receiver.h"
...
ps4::PS4 ps4controller(&Wire);
PS4Receiver receiver;
...
// these don't use joint_servo_mapping, it starts at servo 1 and increments
// TODO express these in angles and translate when required
// const int16_t cw_limit[kNumServos] = {239, 239, 188, 188, 212, 102, 239, 239, 188, 188, 212, 102,
// 239, 239, 188, 188, 212, 102}; const int16_t ccw_limit[kNumServos] = {785, 785, 836, 836, 922,
// 812, 785, 785, 836, 836, 922, 812, 785, 785, 836, 836, 922, 812}; const int16_t
// individual_offset[kNumServos] = {0, 0, -48, 48, -157, 157, 0, 0, -48, 48, -157, 157, 0, 0, -48,
// 48, -157, 157};
const int16_t offset[kNumServos] = {0, 0, -48, 48, -109, 109, 0, 0, -48, 48, -109, 109, 0, 0, -48, 48, -109, 109};
const int16_t rest[kNumServos] = {kServoMiddle, kServoMiddle, 188, 836, 212, 812,
                                  kServoMiddle, kServoMiddle, 188, 836, 212, 812,
                                  kServoMiddle, kServoMiddle, 188, 836, 212, 812};
int16_t current_position[kNumServos];
int16_t goal_position[kNumServos] = {kServoMiddle, kServoMiddle, 188, 836, 212, 812,
                                     kServoMiddle, kServoMiddle, 188, 836, 212, 812,
                                     kServoMiddle, kServoMiddle, 188, 836, 212, 812};

// This contains the servo IDs in the order that they will be extracted from the hexapod
// The hexapod convention is to go leg by leg from lef to right and front to back i.e. front left,
// front right, next row back left etc..
//  and within each leg moving from the base (where it is attached to the body) out towards the foot
//  i.e. base, hip, knee so overall... front left leg base, hip, knee, front right leg base, hip,
//  knee,
uint8_t joint_servo_mapping[kNumServos] = {1, 3, 5, 2, 4, 6, 13, 15, 17, 14, 16, 18, 7, 9, 11, 8, 10, 12};

// converts an index into model order array, to an index into the servo ID order arrays e.g.
// current_position, goal_position
uint8_t modelJointIdxToServoIndex(const uint8_t model_joint_idx) { return joint_servo_mapping[model_joint_idx] - 1; }

uint8_t servoIndexToModelJointIdx(const uint8_t servo_idx) { return joint_servo_mapping[servo_idx]; }

uint8_t servoIndexToServoId(const uint8_t servo_idx) { return servo_idx + 1; }

uint8_t servoIdToServoIndex(const uint8_t servo_id) { return servo_id - 1; }

uint16_t angleToServoUnits(const float angle) { return static_cast<int16_t>(angle * kRadToServoUnit) + kServoMiddle; }
...
void moveToPosition(const uint8_t id, const int16_t goal_position, const uint16_t steps) {
  ...
  //  Serial.print(F("[moveToPosition] current_position "));
...
void getCurrentPhysicalPosition() {
  for (int servo_id = 1; servo_id <= 18; servo_id++) {
    current_position[servo_id - 1] =
        getRegisterWithRetries(servo_id, ax12::RegisterPosition::AX_PRESENT_POSITION_L, 2, 5);
...
void setup() {
  ...
  Serial.println(F("Receiver ready for arming..."));
  while (true) {
    ps4controller.fetchData();
    if (receiver.arm(ps4controller.input)) {
      break;
    }
    delay(update_period);
  }
