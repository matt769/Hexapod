...
Hexapod buildFromURDF() {
    ...
    for (const auto& leg_base: leg_base_links) {
        ...
        body_to_leg_tf.R_ = QuaternionToRotationMatrix(Quaternion(pose.rotation.w, pose.rotation.x, pose.rotation.y, pose.rotation.z));
    ...
    // bit hacky but assume hexapod dimensions based on legs (although the only one that actually matters is depth)
    ...
    for (size_t idx = 0; idx < num_legs; idx++) {
        ...
        std::cout << "Located " << ":" << leg_tfs_for_build[idx].t_.x() << ", " << leg_tfs_for_build[idx].t_.y() << ", " << leg_tfs_for_build[idx].t_.z() << '\n';
